...
function downloadRowAsCSV(item: PaymentItem) {
  ...
  const headers = [
    "Transaction ID", "Payment Type", "Status", "Total",
    "Inspector Share", "Admin Share", "Urgent Fee", "Platform Fee",
    "Homeowner", "Inspector", "Property Address", "Created Date",
  ];
  const values = [
...
function printRowAsPDF(item: PaymentItem) {
  ...
  const statusColor =
    uiStatus === "Paid" ? "#01B664" : uiStatus === "Pending" ? "#DC3545" : "#FA6161";
  const statusBg =
    uiStatus === "Paid" ? "#E9F9F2" : uiStatus === "Pending" ? "#FAE7E7" : "#FFF1F1";
...
export default function PaymentsTable() {
  ...
  // Map UI filter → backend status value
  const backendStatus =
  ...
  const { data: pageData, isLoading, isError } = useGetPaymentsQuery({
  ...
  const payments: PaymentItem[] = pageData?.data ?? [];
  ...
  const { data: allData } = useGetPaymentsQuery({ per_page: 1, page: 1 });
  const { data: paidData } = useGetPaymentsQuery({ status: "paid", per_page: 1, page: 1 });
  const { data: pendingData } = useGetPaymentsQuery({ status: "pending", per_page: 1, page: 1 });
  const { data: failedData } = useGetPaymentsQuery({ status: "failed", per_page: 1, page: 1 });

  const counts = useMemo(() => ({
    All: allData?.total ?? 0,
    Paid: paidData?.total ?? 0,
    Pending: pendingData?.total ?? 0,
    Canceled: failedData?.total ?? 0,
  }), [allData, paidData, pendingData, failedData]);
  ...
  return (
    <div className="w-full bg-white min-h-screen font-roboto my-6 md:my-12 antialiased select-none">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-1">
            {(["All", "Paid", "Pending", "Canceled"] as const).map((f) => (
              <button
                ...
                className={`px-3 py-2 rounded-sm text-sm font-normal leading-5 transition-all shrink-0 cursor-pointer flex items-center gap-1.5 ${activeFilter === f
                    ? "bg-black text-white"
              ...
              >
                ...
                {f !== "All" && (
                  <span
                    className={`min-w-4 h-4 px-1.5 rounded-full flex items-center justify-center text-[10px] text-white ${f === "Paid"
                        ? "bg-emerald-500"
                        : f === "Pending"
                          ? "bg-amber-400"
                          : "bg-red-400"
        ...
        {/* Bulk Export — exports current page's visible rows */}
        <button
                ...
                {payments.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 text-gray-900 text-sm">
                        {item.trx_id}
                      </td>
                      ...
                      <td className="py-3.5 px-4 text-gray-900 text-sm font-medium">
                        ${item.total}
                      </td>
                      <td className="py-3.5 px-4 text-gray-900 text-sm">
                        ${item.admin_share}
                      ...
                      <td className="py-3.5 px-4">
                        <span
                          className={`inline-block px-3 py-1.5 rounded text-[10px] font-bold ${uiStatus === "Paid"
