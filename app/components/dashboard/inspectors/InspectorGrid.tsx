...
import { Search, ArrowUpDown, Download, Mail, Phone, ChevronRight, ChevronDown } from "lucide-react";
...
export default function InspectorGrid() {
  ...
  const [activeTab, setActiveTab] = useState<"All" | InspectorStatus>("All");
      ...
      {filteredAndSortedInspectors.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5">
          {filteredAndSortedInspectors.map((inspector) => (
            <div
              key={inspector.id}
              className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-md transition-all duration-200 flex flex-col justify-between group"
            >
              <div>
                ...
                <div className="flex bg-[#F5F6FA] p-3 rounded-sm items-center gap-3 mb-5">
                  <div className="relative w-11 h-11 shrink-0">
                    ...
                    <span className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-white shadow-sm ${
                      inspector.status === "Active" ? "bg-[#09BD3C]" : "bg-gray-200"
                    //   inspector.status === "Pending Review" ? "bg-amber-500" : "bg-rose-500"
                    }`} />
                  </div>
                ...
                <div className="space-y-4 mb-5">
                  <div className=" ">
                    <span className="text-sm text-gray-900 font-normal leading-5 font-roboto block">Inspection Type</span>
                    <span className="text-sm text-gray-600 font-normal leading-5 font-roboto mt-1 block">{inspector.inspectionType}</span>
                  </div>

                  <div className="pt-2 border-t border-gray-100 pt-4 space-y-2">
                    <div className="flex items-center gap-2 text-sm text-gray-600 font-normal leading-5 font-roboto">
              ...
              <div className="pt-2">
                {inspector.status === "Pending Review" ? (
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button className="border border-gray-200 text-gray-700 hover:bg-gray-50 font-bold text-xs py-2 rounded-xl transition-colors text-center">
                      View Details
                    </button>
                    <button className="bg-[#2563eb] text-white hover:bg-blue-600 font-bold text-xs py-2 rounded-xl transition-colors text-center shadow-sm">
                      Approve
                    </button>
                  </div>
                ) : (
