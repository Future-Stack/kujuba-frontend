...
import { ArrowUpRight, ArrowDownLeft, ChevronDown, ChevronLeft, ChevronRight } from "lucide-react";
import {
  ResponsiveContainer,
  AreaChart,
  Area,
...
const CustomXTick = ({ x, y, payload, activeDay }: any) => {
  ...
  return (
    <g transform={`translate(${x},${y})`}>
      ...
      <text x={0} y={52} textAnchor="middle" fill="#171B1E" fontSize={14} fontWeight={400}>
        {payload.value}
      </text>
    </g>
  );
  ...
  return (
    <div className="bg-white px-4 py-3 rounded-2xl shadow-xl border border-gray-100">
      {payload.map((entry: any) => (
        <div key={entry.dataKey} className="flex items-center justify-between gap-8 mb-1 last:mb-0">
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full" style={{ backgroundColor: entry.color }} />
            <span className="text-sm text-gray-700">
              {entry.dataKey === "receive" ? "Receive Payment" : "Payout"}
            </span>
          </div>
...
const getIcon = (active: boolean) => {
  return active ? (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      <defs>
        <clipPath id="clip0_539_2146"><rect width="24" height="24" fill="white" /></clipPath>
      </defs>
    </svg>
  ) : (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      <defs>
        <clipPath id="clip0_539_2155"><rect width="24" height="24" fill="white" /></clipPath>
      </defs>
    </svg>
  );
};
...
export default function FinanceInsights() {
  ...
  const isDateFilterActive = !!(appliedStart && appliedEnd);
  ...
  const handleViewTypeClick = (type: "monthly" | "weekly") => {
  ...
  const queryParams = isDateFilterActive
    ? { start_date: toApiDate(appliedStart as Date), end_date: toApiDate(appliedEnd as Date) }
    : { range: viewType };

  const { data, isLoading, isError, refetch } = useGetOverviewQuery(queryParams as any);

  useEffect(() => {
    refetch();
  }, [viewType, appliedStart, appliedEnd]);
  ...
  const maxValue = Math.max(
    ...chartData.map((item: any) => Math.max(item.receive || 0, item.payout || 0)),
    1000
  );
  ...
  return (
    <div className="w-full p-4 md:p-6 bg-white rounded-[20px] border border-gray-200 hover:shadow-sm select-none relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
        ...
        <div className="flex items-center gap-5 self-end sm:self-auto">
          <button
            onClick={() => handleViewTypeClick("monthly")}
            className="flex items-center gap-2 text-sm font-semibold cursor-pointer"
          >
            {getIcon(viewType === "monthly" && !isDateFilterActive)}
            <span className={viewType === "monthly" && !isDateFilterActive ? "text-[#717579]" : "text-gray-400"}>
              Current Monthly
            </span>
          </button>

          <button
            onClick={() => handleViewTypeClick("weekly")}
            className="flex items-center gap-2 text-sm font-semibold cursor-pointer"
          >
            {getIcon(viewType === "weekly" && !isDateFilterActive)}
            <span className={viewType === "weekly" && !isDateFilterActive ? "text-[#717579]" : "text-gray-400"}>
              Current Weekly
            </span>
          </button>
        </div>
      ...
      <div className="flex items-center justify-between mb-6 relative">
        ...
        {isCalendarOpen && (
          <div
            ref={modalRef}
            className="absolute top-[36px] left-0 w-[320px] bg-white rounded-3xl border border-gray-100 p-5 shadow-2xl z-50"
          >
      ...
      {/* Chart */}
      <div className="w-full relative" style={{ height: "350px", minWidth: "300px" }}>
