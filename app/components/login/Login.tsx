...
export default function LoginPage() {
  ...
  const handleGoogleCredentialResponse = useCallback(
    async (response: { credential: string }) => {
      ...
      try {
        const res = await googleLogin(id_token).unwrap();
  ...
  useEffect(() => {
    ...
    return () => {
      // Cleanup: cancel any pending prompt
        ...
        <div className="bg-white border border-gray-100 rounded-sm p-5 md:p-10 hover:shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* <button
              ...
              className="w-full border border-[#E5E7EB] hover:bg-[#F9FAFB] rounded-sm py-2.5 flex items-center
                justify-center gap-2 text-sm font-medium text-[#374151] cursor-pointer transition disabled:opacity-60"
            >
              {isGoogleLoading ? (
